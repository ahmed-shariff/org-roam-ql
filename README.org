...
- A ~QUERY~ :: This is a predicate, similar to the predicates in [[https://github.com/alphapapa/org-ql][org-ql]]. Returns all nodes that pass for the given predicate. For example, consider the following call to ~org-roam-ql-nodes~:
  ...
  - ~backlink-from (SOURCE-OR-QUERY &key (TYPE "id") (COMBINE :or))~ :: Similar to ~backlink-to~, tests if there are any backlinks from (aka forwardlinks) the resulting nodes from ~SOURCE-OR-QUERY~.
...
When these sections are configured in ~org-roam-mode-sections~, using ~F~ in ~org-roam-ql-buffer-dispatch~ (bound to ~v~ in ~org-roam~ buffers) would allow specifying a org-roam-ql query which will be used to filter the backlinks/reflinks displayed in the respective sections. After setting a filter, refresh the buffer for it take effect.
