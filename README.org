...
- ~(org-roam-ql-search SOURCE-OR-QUERY DISPLAY-IN &optional TITLE QUERY SUPER-GROUPS)~ - This creates a ~org-ql~ buffer with the nodes of the corresponding ~SOURCE-OR-QUERY~ with ~TITLE~. Additionally, this also takes a ~org-ql~ query, which will be used to further filter the results from the ~SOURCE-OR-QUERY~. ~SOURCE-OR-QUERY~ is the same as ~org-roam-ql-node~. When called interactively. This can also be called interactively. ~DISPLAY-IN~ is expected to be a symbol, either ~org-ql~ or ~org-roam~. If its ~org-ql~, the results from the ~SOURCE-OR-QUERY~ will be displayed in org-ql's agenda buffer. If its ~org-roam~, will be displayed in a org-roam-ql buffer.
