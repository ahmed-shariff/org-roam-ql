...
- [[#usage][Usage]]
  ...
  - [[#filtering-org-roam-buffers][Filtering org-roam buffers]]
- [[#working-with-org-ql][Working with org-ql]]
- [[#tips-and-tricks][Tips and tricks]]

* Screen-shots

You can query org-roam with ~org-roam-ql-search~. The results are displayed in an org-roam-like buffer. org-roam-ql also comes with a transient that can be used to modify the results viewed. The transient can be activated with ~v~. You can modify the title (~t~), view kind (~k~), query (~q~), filter (~F~), sort (~s~) and specify whether the query should be evaluated against the current in-buffer results or against the whole org-roam database (~i~). Refreshing the buffer (~r~) will display the updated results.

[[images/demo7.gif]]

The transient is available in the org-roam buffer as well, which allows you to start a query from the results in the org-roam buffer. You also can view the results in an agenda-like buffer (~S~). When entering a query in any interactive function of org-roam-ql, it also has completion-at-point. ~org-roam-ql~ buffers support bookmarks (set with ~bookmark-set~). For convenience, the transient has a shortcut for that (~b~).
...
* Usage
** Commands/functions
- =org-roam-ql-search (SOURCE-OR-QUERY &optional TITLE FILTER SORT-FN PREVIEW-FN)= :: This is an *interactive* command that creates a ~org-roam-ql~ buffer with the nodes of the corresponding [[#valid-values-for-source-or-query][ ~SOURCE-OR-QUERY~ ]] with ~TITLE~. ~FILTER~ is an additional [[#valid-values-for-source-or-query][query]] which further filters the nodes displayed. An ~org-roam-ql~ buffer is functionally similar to the ~org-roam-buffer~, but allows displaying any list of nodes ([[#screen-shots][see screen-shots above]]). When called interactively, it will prompt for the ~SOURCE-OR-QUERY~ and ~TITLE~. Note that when entering queries interactively either in ~org-roam-ql-search~ or in the transient, you can get completion-at-point with ~tab~.
...
- org-roam-ql-search-backlinks (SOURCE-OR-QUERY &optional TITLE FILTER SORT-FN PREVIEW-FN) :: This is similar to ~org-roam-ql-search~, but displays nodes that have a backlink to any of the nodes [[#valid-values-for-source-or-query][ ~SOURCE-OR-QUERY~ ]] resolves to. You can switch between the results of calling ~org-roam-ql-search~ and ~org-roam-ql-search-backlinks~ with ~k~ in the transient dispatch.

- =org-roam-ql-nodes (SOURCE-OR-QUERY)= :: Given a [[#valid-values-for-source-or-query][ ~SOURCE-OR-QUERY~ ]] , return a list of nodes.
- ~org-roam-ql-agenda-block (QUERY)~ :: Meant to be used in ~org-agenda-custom-commands~ as a user-defined function. Insert items from processing ~QUERY~ (which is a [[#valid-values-for-source-or-query][ ~SOURCE-OR-QUERY~ ]]) into the current buffer. QUERY is the `match' item in the custom command form. Currently this doesn't respect agenda restrictions. Example:
  #+begin_src emacs-lisp
    (setq org-agenda-custom-commands
          ("cr" "Node a" org-roam-ql-agenda-block '(title "Node a")))
  #+end_src
- ~org-roam-ql-nodes-files (SOURCE-OR-QUERY)~ :: Given a [[#valid-values-for-source-or-query][ ~SOURCE-OR-QUERY~ ]], returns a list of files for the nodes. Can be used in ~org-agenda-custom-commands~. Example:
  #+begin_src emacs-lisp
    (setq org-agenda-custom-commands
...
- ~org-roam-ql-register-dblock-column~ :: Register a new column to use with dblock.  When ~IDENTIFIER~ is seen in ~:columns~ of the org-roam-ql dblock (see [[#org-dynamic-block][Org dynamic block]]), ~FN~ will be called with one parameter, an /org-roam-node/.
- org-roam-backlinks-section-with-ql-filter :: Same as ~org-roam-backlinks-section~, but will respect a filter in a ~org-roam~ buffer. See [[#filtering-org-roam-buffers][Filtering org-roam buffers]].
- org-roam-reflinks-section-with-ql-filter :: Same as ~org-roam-reflinks-section~, but will respect a filter in a ~org-roam~ buffer. See [[#filtering-org-roam-buffers][Filtering org-roam buffers]].
** Valid values for ~SOURCE-OR-QUERY~
- A list of ~org-roam-nodes~ :: This should be self-explanatory.
- A list of parameters that can be passed to ~org-roam-db-query~ :: It should be a list of the form ~(QUERY ARG1 ARG2...)~. The result of calling ~org-roam-db-query~ with these parameters should return a list of records where the first element is the ID of a corresponding node. For example:
  ...
  #+end_src
- A SQL statement :: A SQL statement (a string starting with "select")
...
- A ~QUERY~ :: This is a predicate, similar to the predicates in [[https://github.com/alphapapa/org-ql][org-ql]]. Returns all nodes that pass for the given predicate. For example, consider the following call to ~org-roam-ql-nodes~:
  ...
  - ~backlink-to (SOURCE-OR-QUERY &key (TYPE "id") (COMBINE :or))~ :: Tests if the node has a backlink to any of the nodes from resolving ~SOURCE-OR-QUERY~. ~TYPE~ specifies the link type. If ~TYPE~ is nil, it will match to all types of links. ~COMBINE~ can be :and or :or. If ~COMBINE~ is ~:and~, return only nodes that have backward links to all results of ~SOURCE-OR-QUERY~; if ~:or~, return nodes that have backward links to any of them. For example, the query ~(backlink-to (title "Node") :type nil :combine :and)~ will match to all types of backlinks and return nodes that have backlinks to all the nodes ~(title "Node")~ resolve to.
  - ~backlink-from (SOURCE-OR-QUERY &key (TYPE "id") (COMBINE :or))~ :: Similar to ~backlink-to~, tests if there are any backlinks from (aka forward links) the resulting nodes from ~SOURCE-OR-QUERY~.
  ...
  - ~file-atime (COMPARISON TIME-STRING)~ :: Same as ~scheduled~, tests the ~file-atime~ of a node.
  - ~file-atime-is-after (TIME-STRING)~ :: ~TIME-STRING~ is the same as ~scheduled~. This filters for nodes where ~file-atime~ is after ~TIME-STRING~.
  - ~file-atime-is-before (TIME-STRING)~ :: ~TIME-STRING~ is the same as ~scheduled~. This filters for nodes where ~file-atime~ is before ~TIME-STRING~.
  - ~file-mtime (COMPARISON TIME-STRING)~ :: Same as ~scheduled~, tests the ~file-mtime~ of a node.
  - ~file-mtime-is-after (TIME-STRING)~ :: ~TIME-STRING~ is the same as ~scheduled~. This filters for nodes where ~file-mtime~ is after ~TIME-STRING~.
  - ~file-mtime-is-before (TIME-STRING)~ :: ~TIME-STRING~ is the same as ~scheduled~. This filters for nodes where ~file-mtime~ is before ~TIME-STRING~.
...
- ~org-roam-ql-defexpansion (NAME DOCSTRING EXPANSION-FUNCTION)~ :: Adds an ~EXPANSION-FUNCTION~ which will be identified by ~NAME~ in an org-roam-ql query. The ~EXPANSION-FUNCTION~ should take the parameters passed in the query and return values that can be passed to ~org-roam-ql-nodes~.
...
~org-roam-ql~ buffers visualize the node itself. This is unlike ~org-roam~ buffers, which visualize relationships (like ~backlinks~ or ~refs~). To allow better customization, ~org-roam-ql~ has the following customizable variables. These are mostly similar to the ~org-roam~ counterparts:
- ~org-roam-ql-preview-function~ :: This differs from the ~org-roam~ counterpart. The preview function used to generate the content of a node preview in an ~org-roam-ql-mode~ buffer. Before calling this, the point will be set to ~org-roam-node-point~. It will be called with two arguments: (1) the corresponding node, and (2) the source-or-query of the corresponding buffer. See [[#valid-values-for-source-or-query][Valid values for SOURCE-OR-QUERY]] for what the value of source-or-query can be. By default, it calls ~org-roam-preview-function~. Note that this can be set locally to the buffer and will be used to generate the preview when the buffer is refreshed. The value set for this variable can also be overridden by passing it as a parameter to ~org-roam-ql-search~ when called programmatically; in this case, the preview function passed as an argument is set as the buffer-local value for this variable.
- ~org-roam-ql-heading-function~ :: The function used to generate the heading of a node in `org-roam-ql-mode' buffer. Similar to ~org-roam-ql-preview-function~ above, the point will be set to the point of the node and will be called with two parameters: (1) the node, and (2) the source-or-query.
...
** Filtering org-roam buffers
org-roam-ql comes with two alternate functions that can be used with ~org-roam-mode-sections~:
#+begin_src emacs-lisp
  (setq org-roam-mode-sections
        '(org-roam-backlinks-section-with-ql-filter
          org-roam-reflinks-section-with-ql-filter))
#+end_src

When these sections are configured in ~org-roam-mode-sections~, using ~F~ in ~org-roam-ql-buffer-dispatch~ (bound to ~v~ in ~org-roam~ buffers) lets you specify an org-roam-ql query that will be used to filter the backlinks/reflinks displayed in the respective sections. After setting a filter, refresh the buffer for it to take effect.
...
Note that org-ql works only with org entries, i.e., `heading nodes`. Hence, if there are any file nodes in the result, they will not be displayed. To make that clear, when org-roam-ql results are displayed in an org-ql-view buffer, a warning is added to the end mentioning how many file nodes were in the result. If the extension is loaded, you may view the org-roam-ql results with ~Q~ from the org-roam-ql transient. An org-ql-view can be viewed in an org-roam-like buffer with ~R~ from the org-ql-view transient.
